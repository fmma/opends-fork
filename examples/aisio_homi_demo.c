...
main(int argc, char **argv)
{
	...
	e = ds_file_driver_open();
	if (e.err) {
		fprintf(stderr, "FAILED: ds_file_driver_open err=%d\n", e.err);
		return 1;
	}
	...
	ds_file_handle_t h;
	e = ds_file_handle_register(&h, fd);
	if (e.err) {
		fprintf(stderr, "FAILED: ds_file_handle_register err=%d\n", e.err);
		return 1;
	}

	void *gbuf = ds_file_alloc(size);
	if (!gbuf) {
		fprintf(stderr, "FAILED: ds_file_alloc(%zu)\n", size);
		return 1;
	}

	ssize_t n = ds_file_read(h, gbuf, size, 0, 0);
	if (n < 0) {
		fprintf(stderr, "FAILED: ds_file_read rc=%zd\n", n);
		return 1;
	}

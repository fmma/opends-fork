...
#define AISIO_BASE_QPAIRS 1
#define AISIO_DEFAULT_IO_THREADS 1
#define MAX_IO_THREADS 16
#define SYNC_SPLIT_MIN_BYTES (1u << 20)
#define MAX_BUF_ENTRIES 8192
#define DEFAULT_BOUNCE_SIZE (128 * 1024)
#define NVME_MAX_NLB 65536
#define NVME_PRP_PAGE 4096
#define AISIO_DEFAULT_QUEUE_DEPTH 512
#define AISIO_MAX_QUEUE_DEPTH 4096
...
open_device(struct aisio_driver *d, int fd)
{
	...
	int rc = homic_attach_qpair(d->dev_uri,
	                            AISIO_BASE_QPAIRS + d->n_io_threads,
	                            &d->attach_descpath);
	if (rc < 0) {
		fprintf(stderr,
...
start_read_op(struct io_worker *w, struct file_op *op)
{
	...
	bool owned;
	if (op->split) {
		extents = op->split->extents;
		extent_count = op->split->extent_count;
		owned = false;
	} else {
		int frc = aisio_resolve_extents(op->h->fd, &extents,
		                                &extent_count);
		if (frc < 0) {
...
complete_read_op(struct io_worker *w, struct file_op *op)
{
	...
	if (op->split) {
		struct sync_split *sp = op->split;
		if (n < 0)
			atomic_store(&sp->err, (int)-n);
		else
			atomic_fetch_add(&sp->bytes, (long long)n);
		op->state = FILE_OP_FREE;
		atomic_fetch_sub(&sp->remaining, 1);
		return;
	}
...
static int
env_int(const char *name, int def, int lo, int hi)
{
...
static void
aisio_read_config(struct aisio_driver *d)
{
	d->n_io_threads = env_int(ENV_IO_THREADS, AISIO_DEFAULT_IO_THREADS, 1,
	                          MAX_IO_THREADS);
	d->queue_depth =
	        (uint32_t)env_int(ENV_QUEUE_DEPTH, AISIO_DEFAULT_QUEUE_DEPTH, 1,
	                          AISIO_MAX_QUEUE_DEPTH);
	const char *mask = getenv(ENV_CPU_MASK);
	d->cpu_mask = mask && mask[0] ? strtoull(mask, NULL, 0) : 0;
}

static struct io_worker *
route_op(struct aisio_driver *d)
{
...
static ssize_t
submit_sync_split(struct aisio_driver *d, opends_handle_t fh, void *buf_base,
                  size_t size, off_t file_offset, off_t buf_offset, int nslices)
{
	struct sync_split split;
	atomic_init(&split.remaining, nslices);
	atomic_init(&split.bytes, 0);
	atomic_init(&split.err, 0);

	int frc = aisio_resolve_extents(((struct aisio_handle *)fh)->fd,
	                                &split.extents, &split.extent_count);
	if (frc < 0)
		return -(ssize_t)OPENDS_FS_SETUP_ERROR;

	size_t chunk = (size / (size_t)nslices) & ~((size_t)d->lba_size - 1);

	for (int i = 0; i < nslices; i++) {
		size_t off = (size_t)i * chunk;
		size_t len = (i == nslices - 1) ? size - off : chunk;
		...
		struct file_op *op =
		        &w->file_op_queue[head & FILE_OP_QUEUE_MASK];
		op->mode = FILE_OP_SYNC;
		op->is_write = false;
		op->h = (struct aisio_handle *)fh;
		op->buf_base = buf_base;
		op->u.sync.size = len;
		op->u.sync.file_offset = file_offset + (off_t)off;
		op->u.sync.buf_offset = buf_offset + (off_t)off;
		op->split = &split;
		op->state = FILE_OP_PENDING;
		w->queue_head = head + 1;
	}

	while (atomic_load(&split.remaining) != 0)
		sched_yield();

	free(split.extents);

	int err = atomic_load(&split.err);
	ssize_t n = err ? -(ssize_t)err : (ssize_t)atomic_load(&split.bytes);
	if (n < 0)
		fprintf(stderr,
		        "opends_read(size=%zu, off=%ld) split failed: rc=%zd\n",
               ...
               void *buf_base, size_t size, off_t file_offset, off_t buf_offset)
{
	...
	if (!is_write && d->n_io_threads > 1 && d->lba_size &&
		...
		if (nslices > d->n_io_threads)
			nslices = d->n_io_threads;
		if (nslices > 1)
			return submit_sync_split(d, fh, buf_base, size,

...
working branch and its uncommitted changes are never touched. --push then
publishes the branch to origin (default: commit only).
"""
...
def _ensure_branch():
    ...
    commit = _git("commit-tree", tree, "-m", f"init {BRANCH}").stdout.strip()
    if not commit:

...
block) written to artifacts/<backend>_<data_dir>.log. Parsing is
deferred to scripts/bench_report.py so the cijoe step stays oblivious
to filperf's exact output shape.
...
def main(args, cijoe):
    target = (cijoe.getconf("test.nvme_ns") if args.backend == "gds"
              else cijoe.getconf("test.nvme_bdf"))
    ...
    out = Path(cijoe.output_path) / "artifacts" / f"{args.backend}_{args.data_dir}.log"

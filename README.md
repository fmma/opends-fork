...
device and (for GDS tests) an NVIDIA GPU with GDS support.
...
2. Prepare the target (format NVMe, mount, generate test file):

   ```sh
   python scripts/setup_device.py
   ```

3. Sync code and build on the target:
...
4. Run all test suites:
   ...
   This runs both `test_ref.yaml` (reference backend) and
   `test_gds.yaml` (GDS backend). The GDS test reads a file into GPU
   memory via cuFile and compares the output against the reference
   backend.
